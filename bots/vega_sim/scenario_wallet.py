...
def from_config(
        ...
        result.update(
            {
                f"{scenario_name}": ScenarioWallet(
                    ...
                    automated_market_maker=Agent(key_name=f"automated_market_maker", wallet_name=scenario_name),
    ...
    for scenario_name in result:
        ...
        if not scenario_wallet.automated_market_maker.key_name in wallet_keys:
            logging.info(
                f"Creating the {scenario_wallet.automated_market_maker.key_name} key pair in the {scenario_wallet.automated_market_maker.wallet_name} wallet"
            )
            wallet_cli.generate_key(
                scenario_wallet.automated_market_maker.wallet_name,
                scenario_wallet.automated_market_maker.key_name,
            )
